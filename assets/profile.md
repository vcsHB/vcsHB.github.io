## 👋 안녕하세요 개발 고수가 되고싶은 vcs입니다 

유니티, C#으로 게임 개발합니다. 항상 느끼지만 천직같습니다. 소소하게 개인 프로젝트도 굴리고 있습니다.

---

## 💡 관심 분야

<table>
  <tr style="background-color: #21262d;">
    <td style="padding: 15px;">
      <img src="https://img.shields.io/badge/Game_Design-FF5733?style=flat-square&logo=unity&logoColor=white"><br>
      <span style="color: #ffffff;">게임의 재미는 무엇인지 항상 깊게 생각하고 이를 구현으로 이끌어내기 위해 고민합니다.</span>
    </td>
  </tr>
  <tr style="background-color: #21262d;">
    <td style="padding: 15px;">
      <img src="https://img.shields.io/badge/New_Tech_Hunter-338057?style=flat-square&logo=google-cloud&logoColor=white"><br>
      <span style="color: #ffffff;">더 나은 코드, 더 멋진 코드를 갈망하며 새로운 기술을 찾아다닙니다.</span>
    </td>
  </tr>
  <tr style="background-color: #21262d;">
    <td style="padding: 15px;">
      <img src="https://img.shields.io/badge/Indie_Game_Curator-3357FF?style=flat-square&logo=steam&logoColor=white"><br>
      <span style="color: #ffffff;">독창적인 아이디어가 빛나는 인디게임들을 발굴하고, 분석하며, 꿈을 꿉니다.</span>
    </td>
  </tr>
</table>

---

## 🛠 기술 스택 (Tech Stack)

### 🎮 Unity & C#
<div class="tech-stack-badges">
  <img src="https://img.shields.io/badge/Unity-20232A?style=for-the-badge&logo=unity&logoColor=white"> 
  <img src="https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white">
</div>

<table>
  <tr style="background-color: #21262d;">
    <td style="padding: 15px; color: #1af483;">
      <strong>다양한 객체지향 디자인 패턴에 대한 이해도를 가지고 있고 프로젝트에 응용할 수 있습니다.</strong><br><br>
      <ul>
        <li><strong>Advanced Tools:</strong> Unity 6 Behaviour Tree, ShaderGraph, Localization 등 활용</li>
        <li><strong>Versatility:</strong> 장르 불문, 기획부터 구현까지 능숙하고 빠른 개발 프로세스 보유</li>
        <li><strong>Architecture:</strong> 유지보수에 용이한 객체지향적 설계 지향</li>
      </ul>
    </td>
  </tr>
</table>

<br>

<div class="tech-stack-badges">
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white">
  <img src="https://img.shields.io/badge/Windows_API-0078D6?style=for-the-badge&logo=windows&logoColor=white">
</div>

<table>
  <tr style="background-color: #21262d;">
    <td style="padding: 15px; color: #1edaff;">
      <strong>Win32 API 기반의 게임 프레임워크 설계 및 엔진의 핵심 구조를 직접 구현합니다.</strong><br><br>
      <ul>
        <li><strong>Core Engine:</strong> 좌표계 처리, 컴포넌트 패턴 UI, 오브젝트 풀링 등 시스템 자체 제작</li>
        <li><strong>Performance:</strong> 리소스 최적화가 핵심인 2D 탄막 게임 구현 및 메모리 관리 능력 보유</li>
        <li><strong>Networking:</strong> LockStep 기법 기반의 RTS 시뮬레이터 로직 분석 및 유지보수</li>
      </ul>
    </td>
  </tr>
</table>

<br>
<div class="tech-stack-badges">
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black">
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white">
</div>
<table>
  <tr style="background-color: #21262d;">
    <td style="padding: 15px; color: #ffffff;">
      <strong>웹 프론트엔드 기초와 스크립팅을 활용한 데이터 자동화 환경을 구축합니다.</strong><br><br>
      <ul>
        <li><strong>Responsive Web:</strong> 사용자의 환경에 맞춘 유연한 반응형 웹 페이지 UI/UX 구현</li>
        <li><strong>Data Pipeline:</strong> <strong>Google Apps Script(GAS)</strong>를 활용해 외부 데이터관리 자동화</li>
      </ul>
    </td>
  </tr>
</table>