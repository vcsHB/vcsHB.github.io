...
8. 그런데 그 이웃 위치가 아예 갈 예정에도 없었고 가본적도 없는 새로운 위치라면 openSet(새로운 유망 경로)에 추가

을 반복합니다.

### 🔢 우선순위 큐(Priority Queue) 구현

낮은 Score의 위치정보를 먼저 뽑아오기 위하여 우선순위 큐(Priority Queue)를 직접 구현하였습니다. 빠른 길찾기를 위한 수단이지만. 하지만 Enqueue될때마다 정렬을 새로 하기 때문에 성능이 그다지 좋지 못하다는 점이 개발하면서 조금 신경쓰이는 부분이었습니다.

### ↗️ Heuristic에 관하여

<img src="..\assets\ReferenceData\ProjectOxidation\Image_Oxidation_1.png" height="100px" style ="border-radius: 8px;"/>

GetHeuristic함수를 통해 경로 탐색의 최종목표 지점으로의 대략적인 방향을 제시하고 그 방향의 경로를 먼저 탐색 하도록 합니다. 이 코드 내에서는 ‘멘해튼 거리’ 기법을 활용하여 상하좌우의 제한적인 경로 탐색에서 유리하고 효율적인 연산을 할 수 있도록 합니다.

### 🗺️ ReconstructPath에 관하여

<img src="..\assets\ReferenceData\ProjectOxidation\Image_Oxidation_2.png" height="250px" style ="border-radius: 8px;"/>

경로 탐색에 성공하였을때. ReconstructPath()를 해주는 이유는 이후 WaveSystem으로 생성된 에너미들이 해당 경로를 따라 이동하도록 네비게이션 정보(List<Vector2>)를 제공하여야 하는데. 이를 List로 저장해놓고, 에너미가 이를 필요로 할때 얕은 복사로 네비게이션 정보를 전달합니다.

### + 🚨 개발 중 사소한 사건사고

<img src="..\assets\ReferenceData\ProjectOxidation\Image_Oxidation_3.png" height="550px" style ="border-radius: 8px;"/>


이 길찾기 알고리즘을 개발할 적에 목표위치와 벽이 애매하게 가까웠어서 벽인줄 알고 목표지점이 배제되어, 길찾기를 시도하다 스택오버플로우로 게임이 에디터 채로 폭발하는 문제가 발생했었습니다.

<img src="..\assets\ReferenceData\ProjectOxidation\Image_Oxidation_4.png" height="150px" style ="border-radius: 8px;"/>
...
모든 건물들이 공통적으로 가지고 있는 요소인 체력과 건물 이름은 위로 따로 빼고 건물의 종류에 따라 상세한 정보를 표기해야했기 때문에 이에 따른 패널을 분리하여 개발할 필요가 있었습니다. 공장 타입의 건물은 필요한 재료들, 생산에 걸리는 시간, 생산 후 나오는 자원의 타입과 개수들을 보여줄 필요가 있었습니다.  
