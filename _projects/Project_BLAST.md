...
## 💻 주요 기능 개발 리스트
- **BUFF & DEBUFF System** - 버프와 디버프 시스템<br>

- **ALL UIs** - 게임내 전반적인 UI 개발
- **Part Change System** - 파츠 변경 및 저장 시스템
- **PlayerSkill Dev** - 플레이어 스킬 개발
...
## ⚡ BUFF & DEBUFF System - 버프와 디버프 시스템

FSM 구조 에서 일부 영감을 받아 Entity의 버프 디버프 시스템을 구현했습니다. 🔗[깃허브 EffectSystem Folder](https://github.com/GapMoeGroupMeister/BLAST/tree/main/Assets/01.Scripts/EffectSystem)
...
### 🎨 컬러피커 UI 🔗[깃허브 ColorSetting Folder](https://github.com/GapMoeGroupMeister/BLAST/tree/main/Assets/01.Scripts/UI/LobbyScene/PartPanel/ColorSetting)

<img src="..\assets\ReferenceData\ProjectBlast\GIF_Blast_1.gif" height="300px" style ="border-radius: 8px;"/>

 컬러피커 UI는 대회 제출 직전에 개발을 시작해서 완료한 급조된 구조라, 다시 봐도 엄청 비효율적이고 별로 못짠 코드같습니다. 무엇보다 컬러를 찍으면 HSV로 변환하여 각 슬라이더로 표시하고, 슬라이더를 조정하면, 컬러 피커 상에서 색깔이 변경되어야하는 고통스러운 **양방향 input, output**으로 인해 안전한 종속성 유지 개발에 큰 고난을 겪었습니다. 

 추후 심화 개발을 통해 다른 여러 대회에 출품하고, 출시하는 과정에서 새롭게 리팩토링할 예정입니다.
...
### 📋 Player Part 플레이어 파츠 선택 UI

<img src="..\assets\ReferenceData\ProjectBlast\Image_Blast_3.png" height="300px" style ="border-radius: 8px;"/>

 파츠 선택 UI는 이 다음에 설명될 파츠 변경 시스템을 동작시키는 부분으로, **MVP 패턴**을 활용한 대표적인 예시입니다. 

### Model : 데이터를 가지고 있는 부분
...
🔗[깃허브 PartSelectPanel.cs](https://github.com/GapMoeGroupMeister/BLAST/blob/main/Assets/01.Scripts/UI/LobbyScene/PartPanel/PartSelectPanel.cs)  ← (Presenter)
...
🔗[깃허브 SelectDisplayPanel.cs](https://github.com/GapMoeGroupMeister/BLAST/blob/main/Assets/01.Scripts/UI/LobbyScene/PartPanel/SelectDisplayPanel.cs) 🔗[깃허브 PartSelectSlot.cs](https://github.com/GapMoeGroupMeister/BLAST/blob/main/Assets/01.Scripts/UI/LobbyScene/PartPanel/PartSelectSlot.cs)
...
### Presenter : 데이터 가공, View에게 전달하는 부분

Presenter는 위에서 잠깐 나왔듯이 아래의 기능을 그대로 수행합니다. 
...
🔥불이 근처에서 발생하면 기름에 불이 옮겨 붙습니다. 대표적으로는 플레이어의 화염방사 공격, 네이팜 스킬등이 있습니다.
...
### 🔗[그렇게 그에 따른 내용들을 고찰하다가 탄생한 Caster 시스템에 관한 티스토리 글](https://dev-vcs.tistory.com/entry/%EA%B2%8C%EC%9E%84-%EA%B0%9C%EB%B0%9C-Caster-%EC%8B%9C%EC%8A%A4%ED%85%9C-Damage-Knockback)
